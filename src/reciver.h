...
void recive_task(void *parameter)
{
  ...
  const TickType_t QUEUE_TIMEOUT = 500 / portTICK_PERIOD_MS; // 500ms wait
  ...
  while (1)
  {
    ...
    // Inside recive_task:
    if (xQueueSend(queue_aircraft_data, &recived_data, QUEUE_TIMEOUT) != pdTRUE)
    {
...
void actuators_task(void *parameter)
{
  ...
  while (1)
  {
    if (xQueueReceive(queue_aircraft_data, &recieve_data, portMAX_DELAY) == pdTRUE)
    {
